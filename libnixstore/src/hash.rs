...
impl Hash {
    ...
    fn to_base32(&self) -> String {
        nixbase32::to_nix_base32(self.data())
    }

    pub(super) fn from_ffi_hash(hash: FfiHash) -> Result<Self> {
        match hash.type_ {
...
fn decode_hash<'s>(s: &'s str, typ: &'static str, expected_bytes: usize) -> Result<Vec<u8>> {
    ...
    let v = if s.len() == base16_len {
        ...
        nixbase32::from_nix_base32(s).ok_or(Error::InvalidBase32Hash)?
    } else {
